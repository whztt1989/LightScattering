...
void main()
{
	...
	computeInscatteringIntegral(uRayStart, uRayEnd, uEarthCentre, uLightDir, NetParticleFromCam, RayleighInsc, MieInsc, 7);
	vec3 LightInScattering = RayleighInsc + MieInsc;

	gl_FragColor = vec4(LightInScattering*100, 1.0);
	//if (LightInScattering.x < LightInScattering.y)
	//	gl_FragColor = vec4(LightInScattering.x*10, 0.0, 0.0, 1.0);
	//else
